...
def runTimings(mode : str):
    ...
    max_densities = np.power(10,
        np.linspace(np.log10(base_max_density), np.log10(end_max_density), DENSITY_STEPS))
    ...
    min_densities = np.log10(
        np.linspace(np.power(10,base_min_density), np.power(10,end_min_density), DENSITY_STEPS))
    ...
    volume = pyrenderer.Volume.create_implicit(pyrenderer.ImplicitEquation.Cassini, VOLUME_RESOLUTION)
