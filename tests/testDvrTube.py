...
def runTimings(mode : str):
    # CONFIGURATION
    SETTINGS_FILE = "../scenes/dvrTube-scene1.json"
    ROOT_PATH = ".."
    RESOLUTION = (512, 512)
    NUM_FRAMES = 1
    ...
    TIMING_STEPS = 50
    OUTPUT_STATS_ALL = "../results/statistics/dvr-tube/timings-all-%s.tsv"
    OUTPUT_STATS_AVG = "../results/statistics/dvr-tube/timings-avg-%s.tsv"
    OUTPUT_HISTO_ALL = "../results/statistics/dvr-tube/histograms-%s.tsv"
    OUTPUT_HISTO_CFG = "../results/statistics/dvr-tube/histogram-cfg-%s.tsv"
    OUTPUT_STATS_USE_DOUBLE = False
    OUTPUT_IMAGE_PATH = "../results/statistics/dvr-tube/images/"
    OUTPUT_INSTRUMENTATION = "../results/statistics/dvr-tube/instrumentation.tsv"
    ...
    end_max_density = 0.5*(base_min_density+base_max_density) + MIN_DENSITY_DIFFERENCE
    ...
    end_min_density = 0.5*(base_min_density+base_max_density) - MIN_DENSITY_DIFFERENCE
    min_densities = np.exp(
        np.linspace(np.log(base_min_density), np.log(end_min_density), DENSITY_STEPS))
    scaling = (base_max_density-base_min_density) / (max_densities-min_densities)
    ...
    volume = pyrenderer.Volume.create_implicit(pyrenderer.ImplicitEquation.Tube, VOLUME_RESOLUTION)
