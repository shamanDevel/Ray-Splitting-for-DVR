...
def runTimings(mode : str):
    ...
    KERNEL_NAMES = [
        ...
        [("DVR: Fixed step size - trilinear", "stepping 2^-%d"%s, pow(2, -s)) for s in range(2,16)]
    DENSITY_STEPS = 7
    MIN_DENSITY_DIFFERENCE = 0.01 # minimal difference between min and max density
    TIMING_STEPS = 1
    OUTPUT_STATS_ALL = "../results/statistics/dvr-marschner-lobb/timingsConvergence-all-%s.tsv"
    OUTPUT_STATS_AVG = "../results/statistics/dvr-marschner-lobb/timingsConvergence-avg-%s.tsv"
    OUTPUT_HISTO_ALL = "../results/statistics/dvr-marschner-lobb/histogramsConvergence-%s.tsv"
    OUTPUT_HISTO_CFG = "../results/statistics/dvr-marschner-lobb/histogramConvergence-cfg-%s.tsv"
