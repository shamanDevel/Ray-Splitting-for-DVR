...
void main()
{
    ...
    // channel. Front-to-back
    vec4 fragOut = vec4(0.0, 0.0, 0.0, 0.0);
    for( int i = 0; i < count-1; i++ )
    {
#if 0
        //Rectangle rule
        vec3 color = frags[i].color.rgb;
        float opacity = frags[i].color.a;
#else
        //midpoint rule
        vec3 color = 0.5 * (frags[i].color.rgb + frags[i+1].color.rgb);
        float opacity = 0.5 * (frags[i].color.a + frags[i+1].color.a);
#endif
        float stepsize = frags[i+1].depth - frags[i].depth;
        opacity = 1 - exp(-opacity*stepsize); //min(opacity * stepsize, 1.0);
        fragOut.rgb += (1-fragOut.a) * opacity * color;
        fragOut.a += (1-fragOut.a) * opacity;
    }
    fragOut.rgb = xyzToRgb(fragOut.rgb);
