...
// Add multiple sliders on 1 line for compact edition of multiple components
// The limits have to be specified as vector as well
bool ImGui::SliderScalarNVarLimits(const char* label, ImGuiDataType data_type, void* v, int components, const void* v_min, const void* v_max, const char* format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::SliderInt3(const char* label, int v[3], int v_min[3], int v_max[3], const char* format)
{
    return SliderScalarNVarLimits(label, ImGuiDataType_S32, v, 3, v_min, v_max, format);
}
